# Contributing to DroneMobile MQTT Bridge
...
There is no automated test suite for this project yet (see Checks below);
the fastest local loop is running the container against a real DroneMobile
account and a test MQTT broker.
...
DRONEMOBILE_USERNAME=x DRONEMOBILE_PASSWORD=x DRONEMOBILE_COGNITO_CLIENT_ID=x MQTT_PASSWORD=x python -c "import main"
DRONEMOBILE_USERNAME=x DRONEMOBILE_PASSWORD=x DRONEMOBILE_COGNITO_CLIENT_ID=x MQTT_PASSWORD=x python -m pytest -q
```
